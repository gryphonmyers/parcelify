...
Package.prototype.createAllAssets = function( assetTypes, appData ) {
	var _this = this;
	...
	Object.keys( this.assetSrcPathsByType ).forEach( function( assetType ) {
		_this.assetSrcPathsByType[ assetType ].forEach( function( thisAssetSrcPath ) {
			var thisAsset = _this.createAsset( thisAssetSrcPath, assetType, appData );
		} );
	} );
};

Package.prototype.createAsset = function( thisAssetSrcPath, assetType, appData ) {
	var thisAsset = new Asset( thisAssetSrcPath, assetType, _.clone( this.assetTransformsByType[ assetType ] ), _.clone( this.assetTransformOptsByType[ assetType ] ), appData );
...
Package.getOptionsFromPackageJson = function( packageId, packagePath, packageJson, assetTypes, appTransforms, appTransformDirs, callback ) {
    ...
    packageOptions.assetTransformOptsByType = {};
	...
	async.each( assetTypes, function( thisAssetType, nextAssetType ) {
		...
		async.parallel( [ function( nextParallel ) {
            ...
            packageOptions.assetTransformOptsByType[ thisAssetType ] = [];
			...
			async.map( transformNames, function( thisTransformName, nextTransform ) {
                ...
                if( _.isArray( thisTransformName ) ) {
                    packageOptions.assetTransformOptsByType[ thisAssetType ].push(thisTransformName[1]);
                    thisTransformName = thisTransformName[0];
                } else {
                    packageOptions.assetTransformOptsByType[ thisAssetType ].push({});
                }
